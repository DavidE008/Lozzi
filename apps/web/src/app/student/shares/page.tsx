      ...
      <div className="space-y-3">
        {shares.length ? (
          shares.map((share) => {
            ...
            const active = share.status === "active" && !share.revoked_at && !expired;
            return (
              <Card key={share.id} className="shadow-none">
                <CardContent className="flex flex-col gap-4 py-1 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex gap-4">
                    <span className="flex size-10 shrink-0 items-center justify-center rounded-sm bg-lozzi-teal/10 text-lozzi-teal">
